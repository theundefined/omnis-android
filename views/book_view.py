    ...
    async def load_and_populate():
        ...
        try:
            ...
            content_area.controls.append(
                ft.Text(
                    "Błąd: Nieprawidłowy identyfikator książki.", color=ft.Colors.RED
                )
            )
        ...
        if acc_idx >= len(am.accounts):
            content_area.controls.clear()
            content_area.controls.append(
                ft.Text("Błąd: Nieprawidłowy identyfikator konta.", color=ft.Colors.RED)
            )
        ...
        if not cached_data or loan_idx >= len(cached_data.loans):
            ...
            content_area.controls.append(
                ft.Text(
                    "Błąd: Nie znaleziono książki w pamięci podręcznej.",
                    color=ft.Colors.RED,
                )
            )
            log_debug(
                page,
                f"Book view: Loan index {loan_idx} out of bounds or no cache for UID {uid}",
            )
            page.update()
            return
        ...
        # NOTE: Flet serves the 'assets' directory, so the src path is relative to that.
        full_image_path = f"assets/cache/images/{loan.mmsid}.jpg"
        image_src_path = f"cache/images/{loan.mmsid}.jpg"
        ...
        if os.path.exists(full_image_path):
            image_control = ft.Image(
                ...
                error_content=ft.Icon(
                    ft.Icons.BROKEN_IMAGE, size=100, color=ft.Colors.GREY_400
                ),
            )
        else:
            image_control = ft.Icon(ft.Icons.BOOK, size=100, color=ft.Colors.BLUE_800)

        # --- Create list of additional details ---
        additional_details = []
        if hasattr(loan, "book_details") and loan.book_details:
            details = loan.book_details
            if details.publisher:
                additional_details.append(
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.BUSINESS),
                        title=ft.Text("Wydawca"),
                        subtitle=ft.Text(details.publisher),
                    )
                )
            if details.publication_date:
                additional_details.append(
                    ft.ListTile(
                        ...
                        title=ft.Text("Rok wydania"),
                        subtitle=ft.Text(details.publication_date),
                    )
                )
            if details.physical_description:
                additional_details.append(
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.DESCRIPTION),
                        title=ft.Text("Opis fizyczny"),
                        subtitle=ft.Text(details.physical_description),
                    )
                )
            if details.subjects:
                additional_details.append(
                    ft.ListTile(
                        ...
                        title=ft.Text("Tematy"),
                        subtitle=ft.Text(", ".join(details.subjects)),
                    )
                )
            if details.original_title:
                additional_details.append(
                    ft.ListTile(
                        ...
                        title=ft.Text("Tytuł oryginału"),
                        subtitle=ft.Text(details.original_title),
                    )
                )
            if details.isbns:
                additional_details.append(
                    ft.ListTile(
                        ...
                        title=ft.Text("ISBN"),
                        subtitle=ft.Text(", ".join(details.isbns)),
                    )
                )

        full_location = (
            loan.library_name
            + (f"\n{loan.location_name}" if loan.location_name else "")
            + (f" ({loan.sub_location_name})" if loan.sub_location_name else "")
        )
        ...
        content_area.controls.extend(
            [
                ...
                ft.Text(
                    loan.author or "Brak autora", size=18, color=ft.Colors.GREY_700
                ),
                ft.Divider(),
                ft.ListTile(
                    leading=ft.Icon(ft.Icons.CALENDAR_MONTH),
                    title=ft.Text("Termin zwrotu"),
                    subtitle=ft.Text(loan.due_date, size=16, weight=ft.FontWeight.BOLD),
                ),
                ft.ListTile(
                    leading=ft.Icon(ft.Icons.TODAY),
                    title=ft.Text("Wypożyczono"),
                    subtitle=ft.Text(loan.loan_date),
                ),
                ft.ListTile(
                    leading=ft.Icon(ft.Icons.LOCATION_ON),
                    title=ft.Text("Lokalizacja"),
                    subtitle=ft.Text(full_location),
                ),
                ft.ListTile(
                    leading=ft.Icon(ft.Icons.QR_CODE),
                    title=ft.Text("Kod kreskowy"),
                    subtitle=ft.Text(loan.barcode),
                ),
                ft.ListTile(
                    leading=ft.Icon(ft.Icons.ACCOUNT_BOX),
                    title=ft.Text("Konto"),
                    subtitle=ft.Text(account.name),
                ),
                *additional_details,  # Unpack additional details here
                ft.Text(
