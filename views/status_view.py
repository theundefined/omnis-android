...
async def build_status_view(
    ...
    async def load_and_populate():
        ...
        log_debug(
            page,
            f"Status view: data fetch complete. Data is {'present' if cached_data else 'missing'}.",
        )
            ...
            content_area.controls.extend(
                [
                    ft.ListTile(
                        ...
                        title=ft.Text(
                            info.display_name, size=20, weight=ft.FontWeight.BOLD
                        ),
                    ),
                    ft.Divider(height=10),
                    ft.ListTile(
                        leading=ft.Icon(
                            ft.Icons.MONETIZATION_ON, color=ft.Colors.AMBER
                        ),
                        title=ft.Text("Kary"),
                        subtitle=ft.Text(
                            f"{info.fines_amount} {info.fines_currency}", size=18
                        ),
                    ),
                    ft.ListTile(
