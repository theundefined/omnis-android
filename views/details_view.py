...
async def build_account_details_view(
    ...
    async def load_and_populate():
        ...
        log_debug(
            page,
            f"Details view: data fetch complete. Data is {'present' if cached_data else 'missing'}.",
        )
        ...
        if not cached_data or not cached_data.personal_settings:
            content_area.controls.append(
                ft.Text(
                    "Nie udało się pobrać szczegółowych danych konta.",
                    color=ft.Colors.RED,
                )
            )
                                    ...
                                    if "institution" in reg_entry:
                                        ...
                                        if isinstance(insts, list) and insts:
                                            for inst_entry in insts:
                                                if (
                                                    "user_name" in inst_entry
                                                    and inst_entry["user_name"]
                                                ):
            ...
            content_area.controls.extend(
                [
                    ft.ListTile(
                        ...
                        title=ft.Text("Pełna nazwa"),
                        subtitle=ft.Text(full_name),
                    ),
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.FINGERPRINT),
                        title=ft.Text("PESEL"),
                        subtitle=ft.Text(mask_text(pesel, 2, 2)),
                    ),
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.EMAIL),
                        title=ft.Text("E-mail"),
                        subtitle=ft.Text(mask_text(email, 3, 4)),
                    ),
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.PHONE),
                        title=ft.Text("Telefon"),
                        subtitle=ft.Text(mask_text(phone, 3, 2)),
                    ),
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.HOME),
                        title=ft.Text("Adres"),
                        subtitle=ft.Text(f"{addr1}\n{addr2}".strip()),
                    ),
                    ft.ListTile(
                        leading=ft.Icon(ft.Icons.EVENT_AVAILABLE),
                        title=ft.Text("Konto ważne do"),
                        subtitle=ft.Text(expiry),
                    ),
