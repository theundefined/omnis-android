...
import asyncio
import os
from app_state import Account, get_account_cache_path
...
async def build_account_form_view(index, page, am, app_state, navigate_to, show_snack, log_debug):
    ...
    name_input = ft.TextField(
        label="Nazwa profilu (opcjonalne)",
        autofocus=True,
        value=acc.name if acc else "",
    )
    user_input = ft.TextField(
        label="Numer karty / Login", value=acc.username if acc else ""
    )
    pass_input = ft.TextField(
        label="Hasło",
        password=True,
        can_reveal_password=True,
        value=acc.password if acc else "",
    )
    tenant_options = [
        ft.dropdown.Option(text=t["name"], key=str(i))
        for i, t in enumerate(KNOWN_TENANTS)
        if t["base_url"]
    ]
    tenant_dropdown = ft.Dropdown(
        label="Wybierz bibliotekę",
        options=tenant_options,
        value=str(acc.tenant_index) if acc else "0",
    )
    ...
    async def save_click(e):
        ...
        try:
            ...
            # Clear cache for all accounts since we don't know what changed
            # A more refined approach would be to clear only the relevant cache
            for i in range(len(am.accounts)):
                acc_to_clear = am.accounts[i]
    ...
    async def delete_click(_):
        ...
        dlg = ft.AlertDialog(
            ...
            actions=[
                ...
                ft.TextButton(
                    "Tak", on_click=confirm, style=ft.ButtonStyle(color=ft.Colors.RED)
                ),
            ],
        )
        page.dialog = dlg
            ...
            ft.Row(
                [
                    ft.OutlinedButton(
                        ...
                        on_click=lambda _, idx=index: navigate_to(f"/status/{idx}"),
                        expand=True,
                    ),
                    ft.OutlinedButton(
                        "Dane osobowe",
                        icon=ft.Icons.PERSON_SEARCH,
                        on_click=lambda _, idx=index: navigate_to(f"/details/{idx}"),
                        expand=True,
                    ),
                ]
            ),
        ...
        [
            ft.AppBar(
                leading=ft.IconButton(
                    ft.Icons.ARROW_BACK, on_click=lambda _: navigate_to("/settings")
                ),
                title=ft.Text("Edycja" if is_edit else "Dodaj"),
                bgcolor=ft.Colors.SURFACE_CONTAINER_HIGHEST,
            ),
            ft.Container(
                padding=20,
                content=ft.Column(
                    [name_input, tenant_dropdown, user_input, pass_input, *buttons],
                    spacing=15,
                ),
            ),
        ],
