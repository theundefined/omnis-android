...
try:
    from omnis import UserInfo, Loan
except ImportError:
...
class AccountManager:
    ...
    def save_account_data_to_cache(self, account: Account, user_info: UserInfo, loans: List[Loan], personal_settings: Dict[str, Any]):
    ...
    def load_account_data_from_cache(self, account: Account) -> Optional[CachedData]:
        ...
        try:
            ...
            try:
                ...
                loans = [Loan.model_validate(loan_data) for loan_data in cached_content["loans"]]
            except AttributeError:
                # Fallback for Pydantic v1
                user_info = UserInfo.parse_obj(cached_content["user_info"])
                loans = [Loan.parse_obj(loan_data) for loan_data in cached_content["loans"]]

            print(f"Successfully loaded data from cache for {account.name}")
            return CachedData(last_updated=last_updated, user_info=user_info, loans=loans, personal_settings=personal_settings)
        except Exception as e:
